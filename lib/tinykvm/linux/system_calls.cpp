...
#define SYSPRINT(fmt, ...) \
	if (UNLIKELY(cpu.machine().m_verbose_system_calls)) { \
		...
		if (int(regs.sysret()) < 0) fprintf(stderr, "*** ERROR: %d %s\n", int(regs.sysret()), strerror(-int(regs.sysret()))); \
	}
...
void Machine::setup_linux_system_calls(bool unsafe_syscalls)
{
	...
	Machine::install_syscall_handler(
		SYS_read, [] (vCPU& cpu) { // READ
			auto& regs = cpu.registers();
			const int vfd = int(regs.sysarg(0));
			...
			auto bufcount = cpu.machine().writable_buffers_from_range(
				buffers, regs.sysarg(1), regs.sysarg(2));
			...
			SYSPRINT("read(fd=%d (%d), data=0x%llX, size=%llu) = %lld\n",
				fd, vfd, regs.sysarg(1), regs.sysarg(2), regs.sysret());
		});
	Machine::install_syscall_handler(
		SYS_write, [] (vCPU& cpu) { // WRITE
			auto& regs = cpu.registers();
			const int    vfd = regs.sysarg(0);
			const size_t bytes = regs.sysarg(2);
			SYSPRINT("write(vfd=%d, data=0x%llX, size=%zu)\n",
				vfd, regs.sysarg(1), bytes);
			...
			if (vfd == 1 || vfd == 2) {
				if (UNLIKELY(bytes > 1024*64)) {
					...
					SYSPRINT("write(fd=%d (%d), data=0x%llX, size=%zu) = %lld (overflow)\n",
						vfd, vfd, regs.sysarg(1), bytes, regs.sysret());
					return;
				}
			}
			...
			if (vfd != 1 && vfd != 2) {
				...
				const auto bufcount =
					cpu.machine().gather_buffers_from_range(buffers, regs.sysarg(1), bytes);
				...
				const auto g_buf = regs.sysarg(1);
	...
	Machine::install_syscall_handler(
		SYS_close, [] (vCPU& cpu) { // CLOSE
			...
			SYSPRINT("CLOSE(fd=%d (%d)) = %lld\n",
				vfd, real_fd, regs.sysret());
		});
#ifdef SYS_stat
	Machine::install_syscall_handler(
		SYS_stat, [] (vCPU& cpu) { // STAT
			auto& regs = cpu.registers();
			const auto vpath = regs.sysarg(0);
			...
			if (UNLIKELY(!cpu.machine().fds().is_readable_path(path))) {
				...
				SYSPRINT("STAT to path=%s, data=0x%llX = %lld (not readable)\n",
					path.c_str(), regs.sysarg(1), regs.sysret());
				return;
			}

			struct stat vstat;
			const int result = syscall(SYS_stat, path.c_str(), &vstat);
			regs.sysret() = (result == 0) ? 0 : -errno;
			if (result == 0) {
				cpu.machine().copy_to_guest(regs.sysarg(1), &vstat, sizeof(vstat));
			}
			cpu.set_registers(regs);
	...
	Machine::install_syscall_handler(
		SYS_fstat, [] (vCPU& cpu) { // FSTAT
			...
			int fd = regs.sysarg(0);
			try {
				fd = cpu.machine().fds().translate(regs.sysarg(0));
				struct stat vstat;
				regs.sysret() = fstat(fd, &vstat);
				if (regs.sysret() == 0) {
					cpu.machine().copy_to_guest(regs.sysarg(1), &vstat, sizeof(vstat));
				} else {
					regs.sysret() = -errno;
			...
			SYSPRINT("FSTAT to vfd=%lld, fd=%d, data=0x%llX = %lld\n",
				regs.sysarg(0), fd, regs.sysarg(1), regs.sysret());
		});
#ifdef SYS_lstat
	Machine::install_syscall_handler(
	...
	Machine::install_syscall_handler(
		SYS_lseek, [] (vCPU& cpu) { // LSEEK
			...
			try {
				fd = cpu.machine().fds().translate(regs.sysarg(0));
				regs.sysret() = lseek(fd, regs.sysarg(1), regs.sysarg(2));
				if (int(regs.sysret()) < 0) {
					regs.sysret() = -errno;
				}
			} catch (...) {
				regs.sysret() = -EBADF;
			...
			SYSPRINT("lseek(vfd=%lld, fd=%d, offset=%lld, whence=%lld) = %lld\n",
				regs.sysarg(0), fd, regs.sysarg(1), regs.sysarg(2), regs.sysret());
		});
#ifdef SYS_poll
	Machine::install_syscall_handler(
		SYS_poll, [](vCPU& cpu) { // POLL
			auto& regs = cpu.registers();
			const unsigned guest_count = regs.sysarg(1);
			auto *fds = cpu.machine().template writable_memarray<struct pollfd>(regs.sysarg(0), guest_count);
			const int timeout = int(regs.sysarg(2));
			...
			if (host_fds_count == 0) {
				...
				regs.sysret() = poll(host_fds.data(), host_fds_count, real_timeout);
				if (int(regs.sysret()) < 0) {
					regs.sysret() = -errno;
				} else {
					// Copy back the results
					const size_t count = std::min(size_t(regs.sysret()), size_t(host_fds_count));
	...
	Machine::install_syscall_handler(
		SYS_ppoll, [](vCPU& cpu) { // PPOLL
			...
			const int result = poll(host_fds.data(), host_fds_count, timeout);
			if (result < 0) {
				regs.sysret() = -errno;
			} else {
				regs.sysret() = result;
				for (unsigned i = 0; i < host_fds_count; i++) {
					const unsigned index = host_fds_indexes.at(i);
					fds[index].revents = host_fds[i].revents;
				}
			}
			cpu.set_registers(regs);
			SYSPRINT("ppoll(fds=0x%llX, count=%u, timeout=%d) = %lld\n",
	...
	Machine::install_syscall_handler(
		SYS_mmap, [](vCPU& cpu) { // MMAP
			auto& regs = cpu.registers();
			const uint64_t address = regs.sysarg(0) & ~PageMask;
			const uint64_t length = (regs.sysarg(1) + PageMask) & ~PageMask;
			const int prot = regs.sysarg(2);
			const auto flags = regs.sysarg(3);
			if (UNLIKELY(int(regs.sysarg(4)) >= 0))
			{
				// mmap to file fd
				const int vfd = int(regs.sysarg(4));
				const int64_t voff = regs.sysarg(5);
				const int real_fd = cpu.machine().fds().translate(vfd);
				const bool mmap_backed_files = cpu.machine().memory.mmap_backed_files;
				const uint64_t read_length = regs.sysarg(1); // Don't align the read length
				...
				PRINTMMAP("mmap(0x%lX (0x%llX), %lu, prot=%llX, flags=%llX, vfd=%d) = 0x%llX -> 0x%lX\n",
						  address, regs.sysarg(0), read_length, regs.sysarg(2), regs.sysarg(3), int(regs.sysarg(4)),
						  regs.sysret(), cpu.machine().mmap_current());
				return;
			}
			else if (address != 0x0 && address >= cpu.machine().heap_address() && address + length <= cpu.machine().mmap_start())
			{
				...
				if (flags & MAP_FIXED) {
					...
					regs.sysret() = cpu.machine().mmap_allocate(length, prot);
				}
			}
			else if (address != 0x0 && address >= cpu.machine().mmap_start() && address + length <= cpu.machine().mmap_current())
			{
				...
				regs.sysret() = cpu.machine().mmap_fixed_allocate(address, length, flags & MAP_FIXED, prot);
			}
			...
			if ((flags & MAP_ANON) != 0 && regs.sysret() != ~0ULL)
			{
				cpu.machine().memzero(regs.sysret(), length);
			}
			cpu.set_registers(regs);
			cpu.machine().do_mmap_callback(cpu, address, length, flags, prot, -1, 0);
			PRINTMMAP("mmap(0x%lX, %lu, prot=%llX, flags=%llX, vfd=%d) = 0x%llX\n",
					  address, length, regs.sysarg(2), regs.sysarg(3), int(regs.sysarg(4)), regs.sysret());
		});
	Machine::install_syscall_handler(
		SYS_mprotect, [](vCPU& cpu) { // MPROTECT
			/* SYS mprotect */
			auto& regs = cpu.registers();
			const uint64_t address = regs.sysarg(0);
			const uint64_t length = regs.sysarg(1);
			const int prot = regs.sysarg(2);
			...
			cpu.set_registers(regs);
			cpu.machine().do_mmap_callback(cpu,
				address, length, 0, prot, -1, 0);
			PRINTMMAP("mprotect(0x%llX, %llu, 0x%X) = %lld\n",
				(unsigned long long)address, (unsigned long long)length, prot, regs.sysret());
		});
	Machine::install_syscall_handler(
		SYS_munmap, [](vCPU &cpu) { // MUNMAP
			auto& regs = cpu.registers();
			// We don't support MMAP fully, but we can try to relax the mapping.
			const uint64_t old_base = regs.sysarg(0) & ~PageMask;
			const uint64_t old_size = (regs.sysarg(1) + PageMask) & ~PageMask;
			...
			PRINTMMAP("munmap(0x%lX, %lu, relaxed=%d) = %lld (current=0x%lX)\n",
				old_base, old_size, relaxed, regs.sysret(), cpu.machine().mmap_current());
		});
	Machine::install_syscall_handler(
		SYS_brk, [](vCPU& cpu) { // BRK
			auto& regs = cpu.registers();
			const uint64_t old_brk = cpu.machine().brk_address();
			uint64_t new_brk = regs.sysarg(0);
			if (new_brk < old_brk) {
	...
	Machine::install_syscall_handler(
		SYS_rt_sigaction, [](vCPU& cpu)
		{
			/* SYS rt_sigaction */
			auto& regs = cpu.registers();
			const int sig = regs.sysarg(0);
			const uint64_t g_act = regs.sysarg(1);
			const uint64_t g_oldact = regs.sysarg(2);
			//const int flags = regs.sysarg(3);
	...
	Machine::install_syscall_handler(
		SYS_rt_sigprocmask, [](vCPU& cpu)
		{
			/* SYS rt_sigprocmask */
			auto& regs = cpu.registers();
			const int how = regs.sysarg(0);
			const uint64_t g_set = regs.sysarg(1);
			const uint64_t g_oldset = regs.sysarg(2);
			const unsigned size = regs.sysarg(3);
			(void)how;
			(void)g_set;
			(void)size;
	...
	Machine::install_syscall_handler(  // sigaltstack
		SYS_sigaltstack, [](vCPU& cpu)
		{
			...
			SYSPRINT("sigaltstack(ss=0x%llX, old_ss=0x%llx) = 0x%llX\n",
				regs.sysarg(0), regs.sysarg(1), regs.sysret());
		});
	Machine::install_syscall_handler( // ioctl
		SYS_ioctl, [](vCPU& cpu) {
			...
			case TCGETS: // Get terminal attributes
				if (int(regs.sysarg(0)) >= 0 && int(regs.sysarg(0)) < 3)
				{
			...
			case TIOCGPTN: { // Get PTY number
					...
					cpu.machine().copy_to_guest(regs.sysarg(2), &value, sizeof(value));
					regs.sysret() = 0;
				}
				break;
			case FIONBIO: // Set non-blocking I/O
				if (int(regs.sysarg(0)) >= 0 && int(regs.sysarg(0)) < 3)
				{
					...
					int arg = 0;
					cpu.machine().copy_from_guest(&arg, regs.sysarg(2), sizeof(arg));
			...
			case FIONREAD: {
					...
					const uint64_t g_bytes = regs.sysarg(2);
					int bytes = 0;
					if (int(regs.sysarg(0)) >= 0 && int(regs.sysarg(0)) < 3) {
	...
	Machine::install_syscall_handler( // pread64
		SYS_pread64, [](vCPU& cpu) {
			auto& regs = cpu.registers();
			const int vfd = regs.sysarg(0);
			const uint64_t g_buf = regs.sysarg(1);
			const size_t   bytes = regs.sysarg(2);
			const off64_t  offset = regs.sysarg(3);
			...
			SYSPRINT("pread64(fd=%d, buf=0x%lX, size=%zu, offset=%lu) = %lld\n",
					 vfd, g_buf, bytes, offset, regs.sysret());
		});
	Machine::install_syscall_handler( // pwrite64
		SYS_pwrite64, [](vCPU& cpu) {
			...
			const int vfd = regs.sysarg(0);
			const auto g_buf = regs.sysarg(1);
			const auto bytes = regs.sysarg(2);
			const auto offset = regs.sysarg(3);
	...
	Machine::install_syscall_handler( // writev
		SYS_writev, [](vCPU& cpu) {
			...
			const int vfd = regs.sysarg(0);
			const unsigned count = regs.sysarg(2);
			int fd = -1;
			...
			else if (vfd == 1 || vfd == 2)
			{
				...
				{
					g_iovec vec;
					cpu.machine().copy_from_guest(&vec, regs.sysarg(1) + i * sizeof(g_iovec), sizeof(g_iovec));
			...
			else
			{
				...
				if (fd > 0)
				{
					...
					cpu.machine().copy_from_guest(vecs.data(), regs.sysarg(1), count * sizeof(g_iovec));
					std::vector<tinykvm::Machine::Buffer> buffers;
			...
			SYSPRINT("writev(%d (%d), 0x%llX, %u) = %lld\n",
					 vfd, fd, regs.sysarg(1), count, regs.sysret());
		});
#ifdef SYS_access
	Machine::install_syscall_handler(
		SYS_access, [](vCPU& cpu) { // ACCESS
			auto& regs = cpu.registers();
			const uint64_t vpath = regs.sysarg(0);
			std::string path = cpu.machine().memcstring(vpath, PATH_MAX);
			const int mode = regs.sysarg(1);
			if (UNLIKELY(!cpu.machine().fds().is_readable_path(path)))
			{
				regs.sysret() = -EACCES;
			}
			...
			SYSPRINT("access(path=%s (0x%lX), mode=0x%X) = %lld\n",
					 path.c_str(), vpath, mode, regs.sysret());
		});
#endif
	Machine::install_syscall_handler(
		SYS_pipe2, [](vCPU& cpu) { // PIPE2
			...
			const uint64_t g_pipefd = regs.sysarg(0);
			const int flags = regs.sysarg(1);
			int pipefd[2];
			...
			SYSPRINT("pipe2(0x%llX, 0x%X) = %lld\n",
					 regs.sysarg(0), int(regs.sysarg(1)), regs.sysret());
		});
	Machine::install_syscall_handler(
		SYS_mremap, [](vCPU& cpu) { // MREMAP
			auto& regs = cpu.registers();
			auto current = cpu.machine().mmap_current();
			const uint64_t old_addr = regs.sysarg(0) & ~PageMask;
			const uint64_t old_len = regs.sysarg(1);
			const uint64_t new_len = regs.sysarg(2);
			const uint64_t old_page_len = (old_len + PageMask) & ~PageMask;
			const uint64_t new_page_len = (new_len + PageMask) & ~PageMask;
			unsigned flags = regs.sysarg(3);
			...
			PRINTMMAP("mremap(0x%llX, %llu, %llu, flags=0x%X) = 0x%llX\n",
					  regs.sysarg(0), regs.sysarg(1), regs.sysarg(2), flags, regs.sysret());
		});
	Machine::install_syscall_handler(
	...
	Machine::install_syscall_handler(
		SYS_madvise, [](vCPU& cpu) { // MADVISE
			...
			const uint64_t address = regs.sysarg(0);
			const uint64_t length = regs.sysarg(1);
			const uint64_t advice = regs.sysarg(2);
			regs.sysret() = 0;
			if (advice == MADV_DONTNEED)
			{
				cpu.machine().memzero(address, length);
			}
			cpu.set_registers(regs);
			PRINTMMAP("madvise(0x%llX, %llu, 0x%llx) = %lld\n",
					  (unsigned long long)address, (unsigned long long)length,
					  (unsigned long long)advice, regs.sysret());
		});
	Machine::install_syscall_handler(
		SYS_dup, [](vCPU& cpu) { // DUP
			...
			try {
				...
				{
					regs.sysret() = cpu.machine().fds().manage(new_fd, false, false);
				}
			} catch (...) {
				regs.sysret() = -EBADF;
			}
	...
	Machine::install_syscall_handler(
		SYS_dup2, [](vCPU& cpu) { // DUP2
			auto& regs = cpu.registers();
			const int vfd = regs.sysarg(0);
			const int new_vfd = regs.sysarg(1);
			if (vfd == new_vfd)
			{
				...
				SYSPRINT("dup2(vfd=%d (%d), new_vfd=%d (%d)) = %lld\n",
						 vfd, vfd, new_vfd, new_vfd, regs.sysret());
				return;
			}
			int fd = -1;
			int new_fd = -1;
	...
	Machine::install_syscall_handler(
		SYS_socket, [](vCPU& cpu) { // SOCKET
			...
			const int domain = regs.sysarg(0);
			const int type = regs.sysarg(1);
			const int protocol = regs.sysarg(2);
			if (domain != AF_INET && domain != AF_INET6 && domain != AF_UNIX && domain != AF_NETLINK)
			{
				regs.sysret() = -EAFNOSUPPORT;
				cpu.set_registers(regs);
				SYSPRINT("socket(domain=%d, type=%d, protocol=%d) = %lld (EAFNOSUPPORT)\n",
						 domain, type, protocol, regs.sysret());
				return;
			}
			...
			{
				regs.sysret() = cpu.machine().fds().manage(fd, true, true);
			}
			cpu.set_registers(regs);
			SYSPRINT("socket(%d, %d, %d) = %d (%lld)\n",
					 domain, type, protocol, fd, regs.sysret());
		});
	Machine::install_syscall_handler(
		SYS_setsockopt, [](vCPU& cpu) { // SETSOCKOPT
			...
			SYSPRINT("setsockopt(fd=%d, level=%d, optname=%d, optval=0x%lX, optlen=%zu) = %lld\n",
					 fd, level, optname, g_optval, optlen, regs.sysret());
		});
	Machine::install_syscall_handler(
		SYS_getsockopt, [](vCPU& cpu) { // GETSOCKOPT
			...
			const int optname = regs.sysarg(2);
			const uint64_t g_optval = regs.sysarg(3);
			const uint64_t g_optlen = regs.sysarg(4);
			std::array<uint8_t, 256> optval;
			...
			SYSPRINT("getsockopt(fd=%d, level=%d, optname=%d, optval=0x%lX, optlen=0x%lX) = %lld\n",
					 fd, level, optname, g_optval, g_optlen, regs.sysret());
		});
	Machine::install_syscall_handler(
	...
	Machine::install_syscall_handler(
		SYS_listen, [](vCPU& cpu) { // LISTEN
			...
			const int vfd = regs.sysarg(0);
			const int fd = cpu.machine().fds().translate_writable_vfd(vfd);
			const int backlog = regs.sysarg(1);
	...
	Machine::install_syscall_handler(
		SYS_accept4, [](vCPU& cpu) { // ACCEPT4
			...
			const int fd = cpu.machine().fds().translate_writable_vfd(vfd);
			const uint64_t g_addr = regs.sysarg(1);
			const size_t g_addrlen = regs.sysarg(2);
			const int flags = regs.sysarg(3);
			struct sockaddr_storage addr {};
			...
			else
			{
				...
				{
					regs.sysret() = cpu.machine().fds().accept_socket_callback(vfd, fd, result, addr, addrlen);
				}
				else
				{
					regs.sysret() = cpu.machine().fds().manage(result, true, true);
				}
			}
			cpu.set_registers(regs);
			SYSPRINT("accept4(fd=%d (%d), addr=0x%lX, addrlen=0x%lX, flags=%d) = %lld\n",
					 vfd, fd, g_addr, g_addrlen, flags, regs.sysret());
		});
	Machine::install_syscall_handler(
		SYS_getsockname, [](vCPU& cpu) { // GETSOCKNAME
			...
			SYSPRINT("getsockname(fd=%d, addr=0x%lX, addrlen=0x%lX) = %lld\n",
					 fd, g_addr, g_addrlen, regs.sysret());
		});
	Machine::install_syscall_handler(
	...
	Machine::install_syscall_handler(
		SYS_socketpair, [](vCPU& cpu) { // SOCKETPAIR
			...
			const uint64_t g_sv = regs.sysarg(3);
			int sv[2] = { 0, 0 };
	...
	Machine::install_syscall_handler(
		SYS_shutdown, [] (vCPU& cpu) { // SHUTDOWN
			...
			try {
				...
				{
					fd = cpu.machine().fds().translate(vfd);
					regs.sysret() = ::shutdown(fd, regs.sysarg(1));
					if (int(regs.sysret()) < 0)
						regs.sysret() = -errno;
				}
			} catch (...) {
			...
			SYSPRINT("SHUTDOWN(fd=%d (%d), how=0x%llX) = %lld\n",
					 fd, vfd, regs.sysarg(1), regs.sysret());
		});
	Machine::install_syscall_handler(
		SYS_sendto, [] (vCPU& cpu) { // SENDTO
			...
			const int vfd = regs.sysarg(0);
			const uint64_t g_buf = regs.sysarg(1);
			const uint64_t bytes = regs.sysarg(2);
			const int flags = regs.sysarg(3);
			const uint64_t g_addr = regs.sysarg(4);
			const socklen_t addrlen = regs.sysarg(5);
			int fd = -1;
			try {
			...
			SYSPRINT("sendto(fd=%d (%d), buf=0x%lX, len=%lu, flags=0x%X, dest_addr=0x%lX, addrlen=%u) = %lld\n",
					 vfd, fd, g_buf, bytes, flags, g_addr, addrlen, regs.sysret());
		});
	Machine::install_syscall_handler(
		SYS_recvfrom, [] (vCPU& cpu) { // RECVFROM
			...
			const int flags = regs.sysarg(3);
			const uint64_t g_addr = regs.sysarg(4);
			const uint64_t g_addrlen = regs.sysarg(5);
			int fd = -1;
			try {
			...
			SYSPRINT("recvfrom(fd=%d (%d), buf=0x%lX, len=%lu, flags=0x%X, src_addr=0x%lX, addrlen=0x%lX) = %lld\n",
					 vfd, fd, g_buf, bytes, flags, g_addr, g_addrlen, regs.sysret());
		});
	Machine::install_syscall_handler(
		SYS_recvmsg, [] (vCPU& cpu) { // RECVMSG
			...
			const int vfd = regs.sysarg(0);
			const uint64_t g_msg = regs.sysarg(1);
			const int flags = regs.sysarg(2);
			int fd = -1;
			try {
				fd = cpu.machine().fds().translate(vfd);
				struct msghdr msg {};
	...
	Machine::install_syscall_handler(
		SYS_uname, [](vCPU& cpu) { // UNAME
			auto& regs = cpu.registers();
			if (cpu.machine().memory_safe_at(regs.sysarg(0), sizeof(struct utsname)))
			{
				struct utsname uts{};
				...
				cpu.machine().copy_to_guest(regs.sysarg(0), &uts, sizeof(uts));
				regs.sysret() = 0;
			}
			else
			{
	...
	Machine::install_syscall_handler(
		SYS_flock, [](vCPU& cpu) { // FLOCK
			...
			const int vfd = regs.sysarg(0);
			const int fd = cpu.machine().fds().translate(vfd);
			const int operation = regs.sysarg(1);
			if (fd < 0)
			{
				regs.sysret() = -EBADF;
			}
			...
			SYSPRINT("flock(fd=%d (%d), operation=0x%X) = %lld\n",
					 vfd, fd, operation, regs.sysret());
		});
	Machine::install_syscall_handler(
		SYS_fcntl, [](vCPU& cpu) { // FCNTL
			auto& regs = cpu.registers();
			const int vfd = regs.sysarg(0);
			const int cmd = regs.sysarg(1);
			int fd = -1;
			try {
				fd = cpu.machine().fds().translate(vfd);
				regs.sysret() = 0;
				...
				else if (cmd == F_GETFL)
				{
					...
					if (flags < 0)
						...
						regs.sysret() = flags & allowed_flags;
				}
				else if (cmd == F_SETFL)
				{
					const int writable_fd = cpu.machine().fds().translate(vfd);
				...
				else if (cmd == F_GETLK64)
				{
					...
					if (res < 0) {
						...
						cpu.machine().copy_to_guest(regs.sysarg(2), &fl, sizeof(fl));
						regs.sysret() = 0;
					}
				}
				else if (cmd == F_SETLK64)
				{
					const int writable_fd = cpu.machine().fds().translate_writable_vfd(vfd);
					struct flock64 fl{};
					cpu.machine().copy_from_guest(&fl, regs.sysarg(2), sizeof(fl));
	...
	Machine::install_syscall_handler(
		SYS_getcwd, [](vCPU& cpu) { // GETCWD
			auto& regs = cpu.registers();
			const uint64_t g_buf = regs.sysarg(0);
			const size_t buflen = regs.sysarg(1);
			...
			SYSPRINT("getcwd(buf=0x%lX (%s), buflen=%zu) = %lld\n",
					 g_buf, cwd.c_str(), buflen, regs.sysret());
		});
#ifdef SYS_mkdir
	Machine::install_syscall_handler(
		SYS_mkdir, [](vCPU& cpu) { // MKDIR
			auto& regs = cpu.registers();
			std::string path = cpu.machine().memcstring(regs.sysarg(0), PATH_MAX);
			...
			SYSPRINT("mkdir(%s (0x%llX), 0x%llX) = %lld\n",
					 path.c_str(), regs.sysarg(0), regs.sysarg(1), regs.sysret());
		});
#endif
#ifdef SYS_rename
	Machine::install_syscall_handler(
		SYS_rename, [](vCPU& cpu) { // RENAME
			auto& regs = cpu.registers();
			std::string from_path = cpu.machine().memcstring(regs.sysarg(0), PATH_MAX);
			std::string to_path = cpu.machine().memcstring(regs.sysarg(1), PATH_MAX);
			...
			SYSPRINT("rename(%s (0x%llX), %s (0x%llX)) = %lld\n",
					 from_path.c_str(), regs.sysarg(0), to_path.c_str(), regs.sysarg(1), regs.sysret());
		});
#endif
#ifdef SYS_readlink
	Machine::install_syscall_handler(
		SYS_readlink, [](vCPU& cpu) { // READLINK
			...
			std::string path = cpu.machine().memcstring(regs.sysarg(0), PATH_MAX);
			const uint64_t g_buf = regs.sysarg(1);
			const size_t bufsiz = regs.sysarg(2);
			...
			SYSPRINT("readlink(%s (0x%llX), buf=0x%lX, bufsiz=%zu) = %lld\n",
					 path.c_str(), regs.sysarg(0), g_buf, bufsiz, regs.sysret());
		});
#endif
	Machine::install_syscall_handler(
		SYS_fchmod, [](vCPU& cpu) { // FCHMOD
			...
			const int fd = cpu.machine().fds().translate_writable_vfd(regs.sysarg(0));
			const mode_t mode = regs.sysarg(1);
			if (fd >= 0)
			{
				if (fchmod(fd, mode) < 0)
				{
	...
	Machine::install_syscall_handler(
		SYS_unlink, [](vCPU& cpu) { // UNLINK
			...
			SYSPRINT("unlink(%s (0x%llX)) = %lld\n",
					 path.c_str(), regs.sysarg(0), regs.sysret());
		});
#endif
	Machine::install_syscall_handler(
		SYS_unlinkat, [](vCPU& cpu) { // UNLINKAT
			...
			SYSPRINT("unlinkat(%d, %s (0x%llX), %d) = %lld\n",
					 vdirfd, path.c_str(), regs.sysarg(2), flags, regs.sysret());
		});
	Machine::install_syscall_handler(
		SYS_gettimeofday, [](vCPU& cpu) { // gettimeofday
			auto& regs = cpu.registers();
			const uint64_t g_buf = regs.sysarg(0);
			const uint64_t g_tzbuf = regs.sysarg(1);
			struct timeval tv;
			regs.sysret() = gettimeofday(&tv, nullptr);
			if (int(regs.sysret()) < 0)
			{
				regs.sysret() = -errno;
			}
			...
			{
				cpu.machine().copy_to_guest(g_buf, &tv, sizeof(tv));
			}
			cpu.set_registers(regs);
			SYSPRINT("gettimeofday(buf=0x%lX, tzbuf=0x%lX) = %lld\n",
					 g_buf, g_tzbuf, regs.sysret());
		});
	Machine::install_syscall_handler(
	...
	Machine::install_syscall_handler( // prctl
		...
		{
			auto& regs = cpu.registers();
			const int option = regs.sysarg(0);
			if (option == PR_GET_NAME)
			{
				const uint64_t g_buf = regs.sysarg(1);
				const size_t buflen = regs.sysarg(2);
				if (buflen > 16)
					regs.sysret() = -EINVAL;
				else
				{
	...
	Machine::install_syscall_handler(
		SYS_clock_gettime, [](vCPU& cpu) { // clock_gettime
			auto& regs = cpu.registers();
			struct timespec ts;
			clockid_t clk_id = regs.sysarg(0);
			if (UNLIKELY(clk_id != CLOCK_REALTIME))
				clk_id = CLOCK_MONOTONIC;
			regs.sysret() = clock_gettime(clk_id, &ts);
			if (int(regs.sysret()) < 0)
				regs.sysret() = -errno;
			else {
				cpu.machine().copy_to_guest(regs.sysarg(1), &ts, sizeof(ts));
			}
			cpu.set_registers(regs);
	...
	Machine::install_syscall_handler(
		SYS_clock_getres, [](vCPU& cpu) { // clock_getres
			auto& regs = cpu.registers();
			const clockid_t vclk = regs.sysarg(0);
			const address_t g_buf = regs.sysarg(1);
			struct timespec ts;
	...
	Machine::install_syscall_handler(
		SYS_openat, [] (vCPU& cpu) { // OPENAT
			auto& regs = cpu.registers();
			const int vfd = regs.sysarg(0);
			const auto vpath = regs.sysarg(1);
			const int flags = regs.sysarg(2) & (O_CREAT | O_TRUNC | O_APPEND | O_RDWR | O_WRONLY);
...
#endif
					if (fd > 0) {
						regs.sysret() = cpu.machine().fds().manage(fd, false);
					} else {
						regs.sysret() = -errno;
					}
					cpu.set_registers(regs);
					SYSPRINT("OPENAT fd=%lld path=%s (real_path=%s) = %d (%lld)\n",
						regs.sysarg(0), path.c_str(), real_path.c_str(), fd, regs.sysret());
					return;
				} catch (const std::exception& e) {
					SYSPRINT("OPENAT failed: %s\n", e.what());
					SYSPRINT("OPENAT fd=%lld path=%s flags=%X = %d\n",
						regs.sysarg(0), path.c_str(), flags, -1);
					regs.sysret() = -1;
				}
			}
			if (write_flags || regs.sysret() == (__u64)-1)
			{
				try {
					int pfd = cpu.machine().fds().current_working_directory_fd();
					if (vfd != AT_FDCWD) {
					...
					SYSPRINT("OPENAT where=%lld path=%s (real_path=%s) flags=%X = fd %d\n",
							 regs.sysarg(0), path.c_str(), real_path.c_str(), flags, fd);

					if (fd > 0) {
						regs.sysret() = cpu.machine().fds().manage(fd, false, true);
					} else {
						regs.sysret() = -errno;
					}
				} catch (...) {
			...
			SYSPRINT("OPENAT vfd=%lld path=%s flags=%X = %lld\n",
				regs.sysarg(0), path.c_str(), flags, regs.sysret());
		});
	Machine::install_syscall_handler(
		SYS_newfstatat, [] (vCPU& cpu) { // NEWFSTATAT
			...
			const int vfd = regs.sysarg(0);
			const auto vpath  = regs.sysarg(1);
			const auto buffer = regs.sysarg(2);
			int flags = sanitize_at_flags(regs.sysarg(3));
			...
			try {
				...
				if (vfd != AT_FDCWD) {
					// Use existing vfd
					fd = cpu.machine().fds().translate(int(regs.sysarg(0)));
				}
				...
				if (UNLIKELY(!cpu.machine().fds().is_readable_path(path) && !path.empty())) {
					...
					if (regs.sysarg(3) & AT_SYMLINK_NOFOLLOW) {
			...
			SYSPRINT("NEWFSTATAT to vfd=%lld, fd=%d, path=%s, data=0x%llX, flags=0x%X = %lld\n",
				regs.sysarg(0), fd, path.c_str(), buffer, flags, regs.sysret());
		});
	Machine::install_syscall_handler(
	...
	Machine::install_syscall_handler(
		SYS_timerfd_settime, [](vCPU& cpu)
		{
			...
			const int vfd = regs.sysarg(0);
			const int flags = regs.sysarg(1);
			const uint64_t g_new_value = regs.sysarg(2);
			const uint64_t g_old_value = regs.sysarg(3);
			...
			SYSPRINT("timerfd_settime(%d, %d, new_value=0x%lX, old_value=0x%lX) = %lld\n",
				vfd, flags, g_new_value, g_old_value, regs.sysret());
		});
	...
	Machine::install_syscall_handler( // epoll_ctl
		...
		{
			auto& regs = cpu.registers();
			const int epollfd = cpu.machine().fds().translate(regs.sysarg(0));
			const int op = regs.sysarg(1);
			const int vfd = int(regs.sysarg(2));
			const int fd = cpu.machine().fds().translate(vfd);
			const uint64_t g_event = regs.sysarg(3);
			...
			if (epollfd > 0 && fd >= 0 && epollfd != fd)
			{
				...
				{
					auto& ee = cpu.machine().fds().get_epoll_entry_for_vfd(regs.sysarg(0));
			...
			if (UNLIKELY(cpu.machine().m_verbose_system_calls))
			{
				...
				SYSPRINT("epoll_ctl(epollfd=%lld (%d), op=%d, fd=%d (%d), event=0x%lX) = %lld "
					...
					regs.sysarg(0), epollfd, op, fd, vfd, g_event, regs.sysret(),
					event_str.c_str(), event.data.fd, event.data.u32, event.data.u64);
			}
		});
#if defined(SYS_epoll_wait) || defined(SYS_epoll_pwait)
#ifdef SYS_epoll_wait
	Machine::install_syscall_handler( // epoll_wait
		SYS_epoll_wait, [](vCPU& cpu)
#else
	Machine::install_syscall_handler( // epoll_pwait
		SYS_epoll_pwait, [](vCPU& cpu)
#endif
		{
			auto& regs = cpu.registers();
			std::array<struct epoll_event, 128> guest_events;
			const int vfd = regs.sysarg(0);
			const uint64_t g_events = regs.sysarg(1);
			const int maxevents = std::min(size_t(regs.sysarg(2)), guest_events.size());
			const int timeout = regs.sysarg(3);
			...
			if (result > 0)
			{
				...
				// using the wrong return value from regs.sysret().
				regs.sysret() = result;
				if (UNLIKELY(cpu.machine().m_verbose_system_calls)) {
			...
			SYSPRINT("epoll_wait(fd=%d (%d), g_events=0x%lX, maxevents=%d, timeout=%d) = %lld\n",
				vfd, epollfd, g_events, maxevents, timeout, regs.sysret());
		});
#if defined(SYS_epoll_wait) && defined(SYS_epoll_pwait)
	Machine::install_syscall_handler( // epoll_pwait
	...
	Machine::install_syscall_handler(
		SYS_getrlimit, [](vCPU& cpu) { // getrlimit
			auto& regs = cpu.registers();
			[[maybe_unused]] const auto g_rlim = regs.sysarg(1);
			regs.sysret() = -ENOSYS;
			cpu.set_registers(regs);
	...
	Machine::install_syscall_handler(
		SYS_prlimit64, [](vCPU& cpu) { // prlimit64
			...
			SYSPRINT("prlimit64(res=%lld new=0x%llX old=0x%llX) = %lld\n",
					 regs.sysarg(1), newptr, oldptr, regs.sysret());
		});
	Machine::install_syscall_handler(
		SYS_sendmmsg, [](vCPU& cpu) { // sendmmsg
			...
			SYSPRINT("sendmmsg(fd=%d, buf=0x%lX, count=%d) = %lld\n",
					 fd, g_buf, vcnt, regs.sysret());
		});
	Machine::install_syscall_handler(
		SYS_getrandom, [](vCPU& cpu) { // getrandom
			auto& regs = cpu.registers();
			const uint64_t g_buf = regs.sysarg(0);
			const uint32_t bytes = regs.sysarg(1);
			const int flags = regs.sysarg(2) & GRND_NONBLOCK;
	...
	Machine::install_syscall_handler(
		SYS_statx, [] (vCPU& cpu) { // STATX
			...
			const int vfd = regs.sysarg(0);
			const auto vpath  = regs.sysarg(1);
			const auto flags  = sanitize_at_flags(regs.sysarg(2));
			const auto mask   = regs.sysarg(3);
			const auto buffer = regs.sysarg(4);
			std::string path;
			int fd = cpu.machine().fds().current_working_directory_fd();
			...
			SYSPRINT("STATX to vfd=%lld, fd=%d, path=%s, data=0x%llX, flags=0x%X, mask=0x%llX = %lld\n",
				regs.sysarg(0), fd, path.c_str(), buffer, flags, mask, regs.sysret());
		});
	Machine::install_syscall_handler(
		SYS_readlinkat, [](vCPU& cpu) { // READLINKAT
			...
			const int  vfd      = regs.sysarg(0);
			const auto vpath    = regs.sysarg(1);
			const auto g_buffer = regs.sysarg(2);
			const auto g_size   = regs.sysarg(3);
			std::string path;
			try {
				path = cpu.machine().memcstring(vpath, PATH_MAX);
				...
				if (cpu.machine().fds().resolve_symlink(path)) {
					...
					// regs.sysret() (unsigned) could wrap. Preserves the original intent to store
					// the length in regs.sysret() (on success) but avoid a segfault if -1
	...
	Machine::install_syscall_handler(
		SYS_symlink, [](vCPU& cpu) { // SYMLINK
			auto& regs = cpu.registers();
			const uint64_t g_path1 = regs.sysarg(0);
			const uint64_t g_path2 = regs.sysarg(1);
			std::string path1;
			std::string path2;
			try {
			...
			SYSPRINT("symlink(path1=%s, path2=%s) = %lld\n",
					 path1.c_str(), path2.c_str(), regs.sysret());
		});
#endif
	Machine::install_syscall_handler(
		SYS_utimensat, [](vCPU& cpu) { // utimensat
			...
			const uint64_t g_times = regs.sysarg(2);
			const int flags = sanitize_at_flags(regs.sysarg(3));
			(void)flags;
			std::string path;
			...
			if (fd > 0)
			{
				if (g_path != 0x0)
				{
					...
					if (!cpu.machine().fds().is_writable_path(path)) {
						...
						regs.sysret() = utimensat(fd, path.c_str(), times, flags);
						if (int(regs.sysret()) < 0)
							regs.sysret() = -errno;
					}
				} else {
					// Use fd
					regs.sysret() = futimens(fd, times);
					if (int(regs.sysret()) < 0)
						regs.sysret() = -errno;
				}
			}
			...
			SYSPRINT("utimensat(fd=%d, path=%s, times=0x%lX, flags=%d) = %lld\n",
					 fd, path.c_str(), g_times, flags, regs.sysret());
		});
	Machine::install_syscall_handler(
	...
	Machine::install_syscall_handler(
		SYS_fchown, [](vCPU& cpu) { // fchown
			...
			SYSPRINT("fchown(fd=%d, uid=%lld, gid=%lld) = %lld\n",
					 vfd, regs.sysarg(1), regs.sysarg(2), regs.sysret());
		});
	Machine::install_syscall_handler(
		SYS_ftruncate, [](vCPU& cpu) { // ftruncate
			...
			if (fd > 0)
			{
				regs.sysret() = ftruncate(fd, regs.sysarg(1));
				if (int(regs.sysret()) < 0)
					regs.sysret() = -errno;
			}
			else {
	...
	if (unsafe_syscalls)
	{
		...
		[](vCPU& cpu) { // symlink
			auto& regs = cpu.registers();
			std::string target = cpu.machine().memcstring(regs.sysarg(0), PATH_MAX);
			std::string linkpath = cpu.machine().memcstring(regs.sysarg(1), PATH_MAX);
			...
			SYSPRINT("symlink(%s (0x%llX), %s (0x%llX)) = %lld\n",
					 target.c_str(), regs.sysarg(0), linkpath.c_str(), regs.sysarg(1), regs.sysret());
		});
#endif
		...
		[](vCPU& cpu) { // inotify_add_watch
			...
			const int fd = cpu.machine().fds().translate(vfd);
			if (fd > 0) {
				regs.sysret() = inotify_add_watch(fd, path.c_str(), mask);
				if (int(regs.sysret()) < 0)
					regs.sysret() = -errno;
			} else {
				regs.sysret() = -EBADF;
			...
			SYSPRINT("inotify_add_watch(fd=%d, path=%s, mask=0x%X) = %lld\n",
					 fd, path.c_str(), mask, regs.sysret());
		});
	} // if (unsafe_syscalls)
